...
function renderAdminMenuView(container, menu) {
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
      ...
      <div style="display:flex;gap:8px;">
        <button class="btn btn-outline btn-sm" onclick="openEditMenuModal(${menu.id}, '${menu.date}')">✏️ Редактирай</button>
        ...
        <tbody>
          ${withMenu.map(r => `
            <tr>
              ...
              <td>
                <div class="table-actions">
                  <button class="btn btn-outline btn-sm" onclick="openEditMenuModal(${r.menu.id}, '${r.date}')">✏️</button>
...
async function openEditMenuModal(id, dateStr) {
    // Fetch the menu for the date this row actually belongs to (not just
    // whatever date happens to be showing in the date-picker above)
    if (!dateStr) dateStr = dateToStr(adminCurrentDate);
    let menu;
    try { menu = await getMenuForDate(dateStr); } catch { toast("Грешка при зареждане на менюто", "error"); return; }
    if (!menu) return;
